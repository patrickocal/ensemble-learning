...
\newtheorem*{diversity*}{Diversity}
\newtheorem*{4-ind*}{\textit{4}-Independence}
...
\newsavebox\mybox
\newtheorem{taggedtheoremz}{Diversity}
\newenvironment{taggeddiv}[1]
 {\renewcommand\thetaggedtheoremz{#1}\taggedtheoremz}
 {\endtaggedtheoremz}
%-----------axiom environment-------------------------------------------------%
\newtheorem{taggedtheoremx}{Prudence}
\newenvironment{taggedpru}[1]
...
%-----------------------------------------------------------------------------%
%-----------basic axioms
\emph{The basic axioms}~of \gsii, which we rewrite in terms of
{generalization}s, are the following. In each of these axioms, $\ext$ is an
arbitrary $Y$-{generalization} of $\preceq_{\mbbd}$.
\begin{taggedblank}{$\textup{A}0$}[Transitivity axiom for $\ext$]\label{T} For
  every $D \in \dpp$, $\ext_{D}$ is transitive.
\end{taggedblank}
\begin{taggedblank}{$\textup{A}1$}[Completeness axiom for $\ext$]\label{K} For
  every $D \in \dpp$, $\ext_{D}$ is complete.
\end{taggedblank}
\begin{taggedblank}{$\textup{A}2$}[Combination axiom for $\ext$]\label{C} For
  every disjoint $C, D\in \dpp$ and every $x,y\in Y$, if $x \ext_{C} y$ and $x
  \ext_{D} y$, then $x \ext_{C \cup D} y ;$ and if $x \sext_{C} y$ and $x
  \ext_{D} y$, then $x\sext _{C \cup D} y$.
\end{taggedblank}
\begin{taggedblank}{$\textup{A}3$}[Archimedean axiom for $\ext$]\label{A} For
  every disjoint $C,D \in \dpp$ and every $x,y\in Y$, if $x \sext_{D} y$, then
...
\emph{The diversity axioms}~that now follow require that $\mbbd$ is
sufficiently rich to support $Y$-{generalization}s $\ext$ with a variety of
...
\begin{taggeddiv}{}[$k$-Diversity axiom] For every $Y \subseteq X$ of
  cardinality $n= 2, \dots, k$, every regular $Y$-{generalization} $\ext$ of
  $\preceqb _{\mbbd}$ is such that $\countof \total(\ext) = n!$\hskip.5pt.
\end{taggeddiv}
We say \emph{$k$-diversity holds on $Z$} if the axiom holds with $Z$ in the
place of $X$. The following axiom substantively weakens \threediv\ and \emph{a
fortiori} \fourdiv.
\begin{taggedblank}{$\textup{A}4'$}[Partial \threediv]\label{p3d} For every
  $Y\subseteq X$ with cardinality $n = 2$ or $3$, every regular
...
\Cref{eg-rates} (and \cref{eg-zaslavski} of the appendix) exemplify sentiments
satisfying \ref{T}–\ref{p3d}, but not \threediv.  For settings where the basic
axioms hold, the next observation shows that \parthreediv\ is equivalent to
\begin{taggedblank}{\textup{A}$4$}[Conditional-\twodiv]\label{c2d} For every
  three distinct elements $x , y , z \in X$, one of the sets $\{D' : x \prec
  _{D '} y \}$ and $\{D' : y \prec_{D '} x \}$ contains  both $C$ and $D$ such
  that $z \prec _{C} x$ and $x \prec _{D} z$. If $\countof X = 2$, then
  \twodiv\ holds on $X$.
\end{taggedblank}
\begin{observation}\label{obs-c2d} For $\preceq_{\mbbd}$ satisfying
  \ref{T}--\ref{A}, \condtwodiv\ and \parthreediv\ are equivalent.
\end{observation}
...
%\emph{Comparing the complexity of \condtwodiv\ and
...
\begin{appendix}
  ...
  \emph{Construction of {generalization}s of $\preceqb_{\mbbj}$.} We
  ...
  regular, novel and testworthy $Y$-{generalization}s (of $\preceqb_{\mbbd}$ or
  ...
  $Y$-{generalization}s. We now clarify what it means to be a {generalization}
  of $\preceqb_{\mbbj}$.
  ...
  \begin{theoremEnd}[no link to proof]{lemma}[proof on
    \cpageref{proof-nov-iso}] \label{lem-nov-iso} For every $Y \in 2^{X}$,
  ...
  \begin{proofEnd}%[Proof of \cref{lem-nov-iso}]
    ...
    \cref{def-{generalization}Q}. The proof that \cref{item-dimension} of
    ...
    of $\preceqb _{\mbbd}$. Finally, via \cref{def-{generalization}Q}, the
  ...
  $Y$-{generalization}s $\ext$ of $\preceqb  _ \mbbj$.
  ...
  \begin{proofEnd}%[Proof of \cref{lem-axiomsQ}]
    ...
    $1-\lambda = \nu \kappa j$. In fact, since $\lambda \in \mbb Q$, we have
    \begin{linenomath*}
      \begin{equation*}
        K \defeq (1-\lambda ) J +\lambda J ' \in \mbbjpp.
      \end{equation*}
    \end{linenomath*}  Simplifying, we obtain $K = \nu ( \kappa L + L ')$. 
    ...
    applying the definition of $\mu$ yields
    \begin{linenomath*}
      \begin{equation*}
    ...
    D'}$ immediately follows.  Together $x \sext _ {(1 -\mu ) L +\mu L'} y$ and
  ...
  \begin{proof}[Proof of \cref{lem-test-empty-fourdiv}]
    ...
    is nonempty. Let $\hext$ be a testworthy $Y$-{generalization}, so that
    there exists $J$ in $\mbbj$ such that $\hext_{J\times 0}$ is total and
    equal to
    the inverse of $\hextb_{\novel}$. Let $P \defeq \hsextb_{\novel}$ so that
    $P \subsetneq Y^2$. Via \ref{TQ}--\ref{c2dQ}, \cref{prop-insep} applies,
    let
    $\hat{v}^{\dd}$ be the matrix representation of $\hext$ and let
    $\hat{v}^{P}$ denote the restriction of $\hat{v}^{\dd}$ to $P \times
    \mbbtp$.
    % and
    ...
    \begin{proof}[Proof of \cref{claim-test-empty}]
      ...
      $\acute{v}^{\dd}: X^{2} \times \mbbtp \rightarrow \R$ as follows. For
      each $\xy$ in $P$, let
      \begin{linenomath*}
        \begin{equation*}
      ...
      $\mbbt$, $\acute{v}^{\dd}(s) \neq \acute{v}^{\dd}(\novel)$. This allows
      us to appeal to \cref{prop-insep} and take $\aextb$ to be the novel
      ...
      and $\hsextb_{\novel} = \hsextb_{J\times 0}^{-1}$ imply that $\aextb
      _{\novel } = \aextb _{J \times 0}^{- 1}$ since, for every $\xy$ in $P$,
      we have
      \begin{linenomath*}
  ...
  \begin{lemma}\label{prop-central-testworthy} For every ranking $R \in
  ...
  \begin{proof} Let $J \in \mbbj$ such that $\ext_{J}$ is total. Since $Y$ is
    finite, so is the dimension of $S_{\mdoubleplus}$.  Take $L \in
  ...
  \emph{The case where $Y$ has cardinality $3$,} follows from
  the next two lemmas.
  ...
  \begin{proof}[Proof of \cref{lem-Y3-r3}] When $\mathbf r = 3$, $u^{\xy},
    ...
    This would reduce $\lvert\hat{\mc{G}}_{\mdoubleplus}\rvert$ to $6$. To
    ...
    reduce $\lvert\hat{\mc{G}}_{\mdoubleplus}\rvert$ to $4$.
  \end{proof}
  \emph{The case where $Y$ has cardinality $4$.} Note that a
  ...
  arrangement of hyperplanes $\acute{\mc H}_{\mdoubleplus}$ is central. That
  is, recalling \cref{eg-zaslavski}, the associated positive intersection
  semilattice $\acute{\mc L}_{\mdoubleplus}$ is isomorphic to $\acute{\mc L}$. 
  The structure of $\acute{\mc L}_{\mdoubleplus}$ is determined by the rank
  ...
  w)}_{\mdoubleplus}$, $f$ is well-defined. As the quotient of continuous
  ...
  u^{\yz}, L\rangle}$ on $G^{(x, y, z, w)}_{\mdoubleplus}$ and the fact that
  ...
  (\ext)$.  It does so by counting the collection $\mc G_{\mdoubleplus}$ of
  open and connected subsets of $\R^{\mbbtpp} \bs \bigcup \{H_{\mdoubleplus} :
  H_{\mdoubleplus} \in \mc A \}$ are called the \emph{chambers} or
  ...
  we are considering the lattice $\mc L$ or the lattice $\mc L_{\mdoubleplus}$
  respectively. In \gsii, as a consequence of \fourdiv, $\mc H_{\mdoubleplus}$
  ...
  2.3]{stanley2007introduction}. Thus, if $\mc H_{\mdoubleplus}$ is
  \emph{centerless}, then $\mc L_{\mdoubleplus}$ is a meet semilattice with
  ...
  \begin{example}[a comparison of $\mc L$ and $\mc
    ...
    and extend to the remaining pairs in $X^{2}$ using \cref{prop-insep}. Since
    these vectors are pairwise noncollinear and $\countof \mbbt = 2$, the
    associated arrangement $\mc H_{\mdoubleplus} =
    \left\{H_{\mdoubleplus}^{\{x,y\}},
    H_{\mdoubleplus}^{\{y,z\}}, H_{\mdoubleplus}^{\{x,z\}}\right\}$ consists of
    three pairwise disjoint lines that partition $\posreal^{\mbbt}$ and $\mc
    G_{\mdoubleplus}$ has cardinality $4$.  We now confirm this using
    Zaslavski's theorem.
    \begin{figure}
      \begin{center}
        \begin{tikzpicture}
        ...
        \caption{\label{fig-hasse-Y3-r2} The intersection semilattice
            $\mc{L}_{\mdoubleplus} = \mc L \bs A^{\{x,y,z\}}$.}
      \end{center}
    \end{figure}

    In the present setting, $A^{\emptyset}_{\mdoubleplus} = \posreal^{\mbbt}$,
    and, via \cref{eq-mobius}, $\bmu(A^{\emptyset}_{\mdoubleplus}) = 1$. Then,
    since $\posreal^{\mbbt}$ is the unique element in $\mc L_{\mdoubleplus}$
    that (strictly) contains each hyperplane in $\mc H_{\mdoubleplus}$,
    \cref{eq-mobius} yields $\bmu(A) = -\bmu(A^{\emptyset}_{\mdoubleplus})$ for
    each $A \in \mc H_{\mdoubleplus}$.  Now since the hyperplanes in
    $\mc{H}_{\mdoubleplus}$ are pairwise disjoint, there are no further
    elements in $\mc{L}_{\mdoubleplus}$. Thus
    \begin{linenomath*}
      \begin{equation*}
    ...
    $\mc L_{\mdoubleplus}$, since $\{0\} \subset \R^{\mbbt}$ is a subset of
  ...
  \begin{remark}[The relationship between $\mc L$ and $\mc
    L_{\mdoubleplus}$]\label{rem-lattice} Let $\aext$ be a $Y$-{generalization}
    ...
    the case for $\acute{\mc H}_{\mdoubleplus}$ where $\acute{\mc
    H}_{\mdoubleplus}$ is centerless and each of its members is maximal in
    ...
    of any $L \in \acute{\mc L}$ is equal to its counterpart $L _{\mdoubleplus}
    \in \acute{\mc L}_{\mdoubleplus}$ provided the latter exists.  Thus,
    $\acute{\mc L}_{\mdoubleplus}$ and $\acute{\mc L}$ are isomorphic if, and
    only if, $\acute{\mc H}_{\mdoubleplus}$ is centered. For the same reason,
    $\acute{\mc G}_{\mdoubleplus}$ and $\acute{\mc G}$ are isomorphic if, and
    only if, $\acute{\mc H}_{\mdoubleplus}$ is centered.
  \end{remark}

  We now abstract a useful property from \cref{eg-zaslavski}.
  \begin{proposition}[polar opposite rankings]\label{prop-pairwise-extremal} If
  ...
  least one-dimensional, $\acute{A}^{\{x,y\} \{y,z\}}_{\mdoubleplus}$ is
