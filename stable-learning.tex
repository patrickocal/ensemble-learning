...
%-----------------------------------------------------------------------------%
\textsc{Prediction requires imagination}: the capacity to explore the world
beyond the data when the data is insufficient. To paraphrase \citep[pages 9 and
...
\citep{hastie2009elements}. Cases (observations) in the training set resemble
memories whereas cases in the test set resemble instances of the imagination in
the sense that they allow the learner to explore unseen and potentially novel
data.
...
As in
\citet{gilboa2003inductive,billot2005probabilities,gilboa2006empirical,argenziano2019second},
learner's kernel is a subjective assessment
...
\begin{example}[Supervised learning]\label{eg-supervised} For suitable nonempty
  ...
  Y)^{n}$  consisting of $n$ cases: each drawn from some unknown
  probability distribution $\rho$ on $X\times Y$. On the basis of $D^{\star}$,
  the goal of supervised learning is to ``learn'' a function $f_{\rho}: X
  \rightarrow Y$ such that, for any given $x$ in $X$, $f_{\rho}(x)$ provides an
  accurate prediction of the associated value $y$ in $Y$. For each $x$ in $X$,
  ...
  One interpretation of the present paper (and that of \gsii) is that, for the
  current value $x = x_{n+1}$, we provide axioms for estimating of $\rho_x$ on
  the basis of $D^{\star}$. One may apply the model to derive $\rho_x$ via the
  following steps:

  \begin{enumerate}
    \item\label{eg-supervised-resample} Derive a collection $\mc D_n$ of
      training sets by resampling from $D^{\star}$. (To start with, take $n$
      ``basis'' training sets each of which contains just a single copy of some
      case in $D^{\star}$.)
    \item\label{eg-supervised-sentiments}Induce sentiments on $\mc D_n$. That
      is a plausibility ranking on $Y$ for each $D$ in $\mc D_n$.
      % , so that, given
      % $y$ and $y'$ in $Y$, $y \preceq_{D} y'$ if, and only if, $y$ is
      % less plausible than $y'$.
    \item \label{eg-supervised-axioms} Via the \parthreediv\ and \stability\
      axioms, extend sentiments to the set $\mbbd_n$ of every finite resampling
      of $D^{\star}$.
    ...
    \item Derive a collection $v^{\dd}$ of vectors that are orthogonal to the
      ...
      identity: for every $y, y',y'' \in Y$, $v^{\yypp} = v^{\yyp} +
      v^{\ypypp}$.
    ...
    \item \label{eg-supervised-estimate} Via the groupoid identity, derive the
      kernel $\mathbf{v}: Y \times \mbbt \rightarrow \R$ satisfying the
      properties of \cref{thm-main}.
    \item Estimate $\rho_x$ using $\hat\rho: Y \times \mbbd_n \rightarrow \R$,
      $y \times D \mapsto \hat \rho_D(y) = \sum_{c \,\in D} \mathbf{v}(y, c)$, 
...
generalizations, are the following. In each of these axioms, $\ext$ is an
...
the fact that it imposes structure on novel generalizations. The following
