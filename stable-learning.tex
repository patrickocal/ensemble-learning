...
\end{small}
  ...
  \fancyhead[LE,RO]{\small\thepage} \fancyhead[CO]{\small\scshape Prudent
  prediction, stable learning and market efficiency}
...
\title{\MakeUppercase{Stable learning and no-arbitrage pricing:\\ on the basis
of sentiments} \footnote{I thank Itzhak Gilboa for encouraging this project at
an early stage.
...
%\resetlinenumber 
\begin{abstract}
  \input{predictor-fin-abstract.tex}
\end{abstract}
  ...
  \section{Introduction} \label{sec-introduction} \textsc{Prudent prediction
  requires the imagination} to go beyond the current sample of past
  input-output pairs (henceforth past cases). To paraphrase \citep[pages 9 and
  ...
  But what about artificial intelligence and its success in prediction despite
  lacking an ``imagination''?  In machine learning it is standard to split the
  data into a training set and a test set.  We may interpret the training set
  as memory and the test set as the imagination.  In our framework, the
  prudent learner takes an within-sample perspective \citep[page 7, experiment
  1]{chervonenkis-recollections} and considers potential for generalizing to
  richer samples of past cases.  The axioms we provide are agnostic in relation
  to the method of external validation. These axioms allow an observer to
  determine whether or not the learner is indeed able to generalise to richer
  samples of past cases in a consistent manner.
 ...
 In statistical learning, the key to generalization is stability
  ...
  present framework allows for more abstract goals. To account for agents that
  are sentient such as humans or mice, the present framework takes
  \emph{sentiments} as primitive.  That is, following \citet[henceforth
  \gsii]{GS_Inductive_inference}, the primitive to which our axioms apply is a
  mapping from samples of past cases to rankings of eventualities.
  % [``A central question for the theory is to determine conditions under which
  % a learning algorithm will generalize from its finite training set to novel
  % examples.'']
  ...
  %]{GS_Inductive_inference} kernels with this property satisfy the Jacobi
  %equations).
...
%\citet[henceforth \gsii]{GS_Inductive_inference}.  
...
%prediction \citep[for a recent review]{MM-Memory_imagination_prediction}, one
...
%leave-one-out cross-validation which \citep{AG-Second-order_induction}
...
%of \citet[henceforth \gsii]{GS_Inductive_inference}.
% 
%  
%
...
% \citep{Cowles-Forecasting,Fama-market-efficiency,Malkiel-market-efficiency,White-Data_snooping,FF-Luck_vs_skill,HL-Lucky_factors,GKX-empirical_asset_pricing_via_ML}.
...
% often, out-of-sample \citep{MN-big_data,BH-out_of_sample}.  
% 
%
...
\end{example}
  ...
  Suppose Inny and {Pru} are potential representative agents in a
    ...
    %(See
    %\citet{GLS-empirical-similarity} for a related axiomatic derivation for
...
% \gsii]{GS_Inductive_inference} is our starting point. 
% 
...
%\citet{Fama-market_efficiency,Malkiel-market_efficiency}. Those notions of
...
%%of \citet{AG-Second-order_induction}. 
%
...
%The rankings map is the mathematical object to which the axioms apply. It
%corresponds to what \citet{Mukherjee-Poggio} refer to as the learning
...
\begin{remark*}[on zero-coupon bonds]
 ...
 % (In this case, via options markets, we may identify suitable
 % benchmarks for calculating the present value of dividends.)
\end{remark*}
...
accumulating interest over $(y, x)$ simply means selling $(x, y)$.  Holding
  ...
  with the market, then her rankings map is centered as in \cref{fig-tali}.
...
\begin{proof}[Proof of \cref{cor-bond-rep}]
  Via \cref{cor-foureq}, $\preceq_{\mbbd}$ satisfies \stability\ if, and only if, there exists a pairwise Jacobi representation
  $v^{\dd}: X^{2} \times \mbbc \rightarrow \R$.
...
For the proof of \eqref{eq-bond-rep}, suppose that \withoutlog, $x \preceq_{D}
  ...
  have $ \sum_{c\,\in D} \left\{ v^{(y,0)} + v^{(0,x)} \right\} \leq 0$.
  Via the representation property $v^{(y, y)} = 0$.
  Another application of the Jacobi identity yields $v^{(y, 0)} = -v^{(0, y)}$. Thus, 
\begin{linenomath*}
  \begin{equation*}\textstyle - \log B(x,D) + \log B(y,D) = \frac{1}{\lvert D\rvert}
  \sum_{c\,\in D} \left \{v^{(0, x)}_{c} - v^{(0, y)}_{c}\right \} \leq 0.  \end{equation*}
\end{linenomath*}
...
the usual \citep{Fama-market-efficiency} sense: passive (buy-and-hold)
...
\citep{Poggio,Mukherjee-Poggio,Bousquet-Elisseeff}. That is to say it implies
continuity of the learning map (here induced by the ranking map)
$L:\bigsqcup_{i \geq 1} Z^n \rightarrow \mc H$ from the sample space to the
hypothesis space \citep{Mukherjee}.  
...
\Citet[p.60]{Malkiel-market_efficiency} points out that ``Markets can be
...
%%\citet[p.10324]{AG-Second-order_induction} describe. 
%In \citet{AG-Second-order_induction}, the ``leave-one-out'' technique of
...
We now restate the axioms for $Y$-{generalization}s $\ext$ of $\preceqb  _ \mbbj$.
...
\begin{step}[for $\countof \mbbt < \infty$, we characterise \ref{KQ}–\ref{AQ},
  \twodiv\ and novel {generalization}s]\label{step-twodiv}
  For any pair of vectors $\acute v, J: \mbbtpp \rightarrow \R$ and consider the
  inner product
\begin{linenomath*}
  \begin{equation*} \langle \acute v, J \rangle \defeq \sum_{t \,\in \,\mbbtpp} v(t) J(t).\end{equation*}
\end{linenomath*}
...
$\acute F^{\xy}_{\mdoubleplus}$ respectively.
    ...
    ``the arrangement generated by a {generalization}''.
...
%  The first lemma in the proof of \cref{thm-mainQ} builds on lemma 1 of \gsii.
 \begin{theoremEnd}{lemma}[two-diverse pairwise representation]\label{lem-insep}
   ...
   Let $\aext$ be a $Y$-{generalization} of $\preceq_{\mbbj}$.  $\aext$ satisfies
...
\begin{proofEnd}%[Proof of \cref{lem-insep}]
  ...
  We now prove the characterisation of novel {generalization}s. Let $\ext$ be a novel
  $Y$-{generalization} with matrix representation $v^{\dd}$ satisfying parts
  \ref{K-insep}–\ref{unique-insep} of the lemma.  Fix arbitrary
  $t \neq \novel$.  Then \cref{def-{generalization}Q} implies the existence of
 ...
 $. Equivalently,
\begin{linenomath*}
  \begin{equation*} \langle v^{\xy}, L + \delta_ t \rangle \leq 0 <\langle v^{\xy}, L + \delta_
...
respect to a given {generalization} $\ext$ and, in addition, $x=y$ implies
...
desired equalities $\beta = \beta'$ and $\sigma = \sigma '$.
  ...
  $\hext$.
\end{proof}
Our proof of \cref{claim-induction-well-defined} shows that the {generalization} to $W$
...
\begin{linenomath*}
    \begin{equation*}v^{\xy}= v^{\xw}+v^{\wy} = -\mathbf{v}(x,\cdot) + \mathbf{v}(y,\cdot).\end{equation*}
\end{linenomath*}
To see that
...
\begin{linenomath*}
  \begin{equation*}0 \not \leq v^{\xy} = - \mathbf{v}(x,\cdot) + \mathbf{v}(y,\cdot).\end{equation*}
\end{linenomath*}  
...
\begin{linenomath*}
  \begin{equation*}\langle v, J\rangle = \sum_{\{t:J(t)>0\}}v(t)J(t).\end{equation*}
  \end{linenomath*}
  ...
  be an improper $Y$-{generalization} of $\preceq_{\mbbj}$ and note that, for every
...
It then follows that, for every pair of vectors $v,J \in
  \R^{n}$, we have
\begin{equation*} \langle T(v) , T(J) \rangle_{S} = \sum_{i,j = 1}^{n} v_{i}J_{j} \,\langle  T_{i},
...
denote the {generalization} generated by $\mathbf{\acute{u}}^{\dd}$ in
...
$Y$-{generalization}s $\ext$ of $\preceq_{\mbbj}$ for $\countof Y = 2,3$ or $4$.  By
...
testworthy {generalization}s with a central arrangement is always nonempty.%  (The
...
\begin{proof} Let $J \in \mbbj$ such that $\ext_{J}$ is total. If, for some
  ...
  let
  \begin{linenomath*}
  \begin{equation*}\acute{\eta}^{\xy} := -\frac{1- \acute{\iota}}{\acute{\iota}}\langle
    u^{\xy}, J \rangle,
  \end{equation*}
  \end{linenomath*}
  and let $\acute{u}^{\xy}\defeq u^{\xy}\times \acute \eta^{\xy}$, so that
  $\langle \acute{u}^{\xy}, \acute{J} \rangle = 0$. Let $\aext$ be the
  associated $Y$-{generalization}, so that by construction
...
\paragraph{The case where $Y$ has cardinality
  ...
  \begin{lemma}\label{lem-Y3-r2} 
    ...
    2$, then \fourjac\ and \stability\ hold on $Y$.
  \end{lemma}
...
$\ext$. (When $\lvert Y \rvert =
  3$, \threejac\ and \fourjac\ coincide.) Also, for \stability, if $\hext$ is any novel, testable {generalization} that satisfies $\hextb_{\novel}=\extb_{J}^{-1}$, then
$\aext$ is a {{diverse}} perturbation of
...
\begin{proof}[Proof of \cref{lem-r3-Y4}]
  ...
  It remains for us to verify that \stability\ also holds. We will show that, for
  every $J\in \mbbj$ such that $\ext_{J}$ is total, the $Y$-{generalization} $\aext$ of
  ...
  maximal and, if $\cext$ is any novel, testworthy {generalization} that satisfies
  $\cextb_{\novel }=\extb_{J }^{-1}$, then $\aext$ is a {{diverse}}
