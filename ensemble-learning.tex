...
\newcommand{\Condthreediv}{\textup{Conditional-\textsc{iii}-diversity}}
\newcommand{\twodiv}{2-\textup{diversity}}
\newcommand{\twodivhash}{2-\textup{Div}$^{\hash}$}
\newcommand{\twodivstar}{2-\textup{Div}$^{*}$}

\newcommand{\fourdiv}{4-\textsc{diversity}}
\newcommand{\fourdivhash}{4-\textup{Div}$^{\hash}$}
\newcommand{\fourdivstar}{4-\textup{Div}$^{*}$}

\newcommand{\threediv}{3-\textsc{diversity}}

\newcommand{\fourjac}{\textup{4-Jac}}
\newcommand{\threejac}{\textup{3-Jac}}
...
\newtheorem*{condtwodiv*}{Conditional-2-diversity axiom}
\newtheorem*{parthreediv*}{Partial-3-diversity axiom}
...
\lccode`\~=`\ %
\lowercase{%
  \gdef\countabstract{\setcounter{word}{0}%
    \catcode`~=\active
    \def~{\space\stepcounter{word}}}}%
\endgroup
\newcounter{word}
\def\endcountabstract{\stepcounter{word}%
...
%The memory and the imagination go hand in hand: they complements in
%prediction.
...
%-----------------------------------------------------------------------------%
\emph{The primitives} consist of the nonempty sets $X$ and $\mbbcp$.  For the
current prediction problem, we interpret members of $X$ as \emph{eventualities}
and members of $\mbbcp$ as \emph{current} cases. We depart from \gsii\ by
allowing for two forms of current case: $\mbbcp$ denotes the union $\mbbc \cup
[\novel]$ of a set $\mbbc$ of constant, \emph{past} cases and a set $[\novel]$
of copies of the variable, \emph{free} case $\novel$.
\begin{remark*}
  ...
  $c\in \mbbc$, similar to \gsii, the content of $m$ is meaningful.  For $c \in
  [\novel]$, although $m$ is empty (assigned a ``garbage'' value), the
  allocation is itself valuable for analysing potential generalizations. Pairs
  $c, d\in [\novel]$ are indistinguishable in terms of content and are in this
  sense copies of $\novel$.
  %and before
...
same \emph{case type} if, and only if, the marginal information of $c$ is
everywhere equal to the marginal information of $d$.  Formally, $c \sim
^{\star} d$ if, and only if, for every $D \in \mbbd$ such that $c , d \notin
D$, $\preceqb_{D \cup \{c\}} = \preceqb_{D \cup \{d\}}$.  By observation 1 of
\gsii, $\sim^{\star}$ is an equivalence relation on $\mbbc$. The equivalence
classes of $\sim^\star$ generate a partition $\mbbt$ of case types.  We extend
$\sim^{\star}$ to $\mbbcp$ by taking $[ \novel ]$ to be an equivalence class of
its own, so that, for every $c \in \mbbc$, $c \nsim^{ \star} \novel$.  We let
$\mbbtp$ denote the corresponding partition of $\mbbcp$.  Like \gsii, we also
extend $\sim^{\star}$ to $\mbbdp$ by treating databases that contain the same
number of each case type as equivalent. That is, $C \sim^{\star} D$ if, and
only if, for every $t \in \mbbtp$, the numbers $\countof (C \cap t)$ and
$\countof (D \cap t)$ of that case type coincide. To
...
\end{assumption*}
\begin{remark*}
  %Just as dummy variables are used to capture intrinsic features of data
  %points
  %such as gender or age group, w 
  We may view case types as being endogenously generated by factors, features
  or hypotheses (\ie\ base learners) that turn out to be relevant to the
  learning problem.
  % That is, write cases as one-hot encodings, so that $c\in t$ if, and only
  % if, there
  % exists $f : \mbbt \rightarrow \{0, 1\}$ such that $f(t') = 1$ only if $t' =
  % t$. 
  For instance, with a yield curve that is characterised by three factors (\eg\
  level, slope and curvature), partition each dimension into suitably small
  intervals and allocate daily observations according to the observed value of
  each of the three factors for a given day's yield curve: then $c\in t$ if,
  and only if, $c$ belongs to the corresponding cuboid in $\R^3$. Resampling
  (with replacement) and possibly perturbing ensures the richness assumption is
  satisfied.
\end{remark*}
...
\emph{Potential generalizations} of sentiments $\precsimb_\mbbd$ to $\mbbdp$
require a suitable structure.
%The next definition provides that structure.
%whilst also simplifying the exposition: by allowing us to restrict attention to
%subsets $Y$ of $X$ and to improper generalizations $\preceqb_\mbbd \cap Y^2$.
\begin{definition}\label{def-generalization} $\extb \defeq \langle \extb_{D}:
  D\in \mbbdp \rangle$ is a \emph{generalization} or $Y$-generalization of
  $\preceq_{\mbbd}$ if, for some nonempty $Y \subseteq X$, both the following
  hold$:$
  \begin{enumerate}%[label=\textup{(\roman*)}]
    \item\label{item-binary-rel} for every $D\in \mbbdp$, $\extb_{D} \subseteq
      Y^2$.
    \item \label{item-dimension} for every $D\in \mbbdp$ and every $c,d \in
      \cpp\bs D$,  if $c \sim^\star d$ then $\extb_{D \cup \{c\}} = \extb _ {D
      \cup \{d\}}$.
      %    \item \label{item-preserving} for every $D \in \mbbd$ and every $x,y\in Y$,
      %      $x \ext_{D} y$ if, and only if, $x \preceq_{D} y$,
  \end{enumerate}
%  $\ext$ is \emph{proper} if it is defined on $\dpp = \mbbdp$ and improper
%  otherwise. For any proper
  For any generalization $\ext$, we write the restriction of
  $\ext$ to $\mbbd$ as $\extb_\mbbd = \langle \extb_{D}: D \in \mbbd \rangle$.
  We then refer to $\aextb = \extb_\mbbd$ as an improper generalization.
  % , for some $Y$ and
  % every $D \in \mbbd$, $\extb_{D} = \preceqb_{D}\cap Y^{2}$.
  %Let $\ext^\star$ denote the unique (improper) generalization
\end{definition}

Let $\ext$ be a $Y$-generalization. Part \ref{item-binary-rel} ensures that
$\ext_{D}$ is a well-defined binary relation on $Y$ for
every $D \in \mbbdp$. We let $\next_{D}$ and $\sext_{D}$ respectively denote
the symmetric and asymmetric parts of $\ext_{D}$. Part \ref{item-dimension} is
a \emph{no-underfitting condition}.  It requires that the partition $\mbbtp$ of
case types generated by $\sim^{\star}$ is at least as fine the partition
generated by the equivalence relation generated by $\ext$. Two cases $c,d \in
\mbbcp$ are \emph{equivalent with respect to $\ext$}, written $c \sim^{\extb}
d$, if, for every $D \in \mbbd$ such that $c,d \notin D$, $\extb_{D \cup \{c\}}
= \extb_{D \cup \{d\}}$.
%Observe that, since $[\novel]$ is a distinct equivalence class of $\sim^{*}$,
%no-underfitting allows for the possibility that $\novel \sim^{\supext} c$ for
%some $c \in \mbbc$.  novel vs regular
\begin{definition*}\label{def-novel} A proper generalization $\ext$ is either
  ...
  \mbbc$, $c \nsim^\supext \novel$.
\end{definition*}
For any given novel generalization $\ext$, $[\novel]$ is the unique novel case
type that $\sim^{\extb}$ generates. We restrict attention to generalizations of
this form, not because we think the imagination or test set of learners is
constrained in this way, but because one degree of freedom is a minimal
requirement for our purposes. Indeed, in the case of small data sets, it is
common to use leave-\emph{one}-out methods where the test (or development)
set contains just one sample.

%It ensures that, for every $D \in \mbbd$, $\ext_{D}$ is the restriction
%$\preceqb_{D}\cap Y^{2}$ of $\preceq_{D}$ to $Y$. This restricts our model of
%the learner to the set of "good" generalizations: where there is no
%overfitting on the training set. These are generalizations where there is
%acceptance of $\preceq_{D}$. 
%is the final one in an iteration of training-validation steps over data that 
%If and when the learner is exposed to the test set there will be the option, but
%not the obligation, to maintain current sentiments. Since the contents of the
%test set are unknown, it follows that it may contain cases that are
%equivalent (with respect to a given generalization $\ext$) to some case in
%the training set, or it may contain a case that is novel.
%We refer to generalizations that satisfy part \ref{item-preserving} as
%stable.  We also refer to this condition as the preservation or
...
%Moreover, it may be that the test set only has a single novel example/feature.
%{Pru} represents a minimal deviation from Inny.
...
%-----------------------------------------------------------------------------%
\subsection{The diversity axioms}

%\begin{axioms*}
%  For every $D \in \mbbd$, and every regular $Y$-generalization $\ext$,
%  $\extb_D = \preceqb_D \cap Y^2$.
%\end{axioms*}

\emph{The diversity axioms} apply to improper generalizations such as $\extb =
\langle \preceqb_D \cap Y^2 : D \in \mbbd \rangle$. (Let $\preceqb_\mbbd \cap
Y^2$ be shorthand for generalizations of this form.) Diversity axioms require
that $\mbbd$ is sufficiently rich to support a variety of \emph{total}
orderings: \ie\ complete, transitive and antisymmetric
($x \ext_{D} y$ and $y \ext_{D} x$ implies $x = y$). %\label{sec-div-pru}
For any $Y$-generalization $\ext$, let $\total(\ext)\subseteq \relations(Y)$
denote the set of total orderings that feature in $\ext$.
%Thus $\total(\ext) = \lb r : \text{for some $C \in \dpp$, $r = \extb_{C}$ is total}\rb$.
For $k = 4$, the following axiom is a restatement of the diversity axiom of
\gsii.
\begin{diversity*} For every $Y \subseteq X$ of cardinality $n= 2, \dots, k$,
  %  and every generalization $\ext$ with $\extb_\mbbd = \preceqb_\mbbd \cap Y^2$,
  the improper $Y$-generalization $\extb = \preceqb_\mbbd \cap Y^2$ satisfies
  $\countof \total(\ext) = n!$\hskip.5pt.
\end{diversity*}
We introduce the following axioms:
%  \settheoremtag{par-3-div}
\begin{taggedp3d}{\textsc{p3diversity}}\label{p3d} For every $Y\subseteq X$ of
  cardinality $n = 2$ or $3$ the improper $Y$-generalization $\extb =
  \preceqb_\mbbd \cap Y^2$ satisfies $\countof{\total(\ext)} \geq n$.
\end{taggedp3d}
...
\begin{observation}\label{obs-c2d} For $\preceq_{\mbbd}$ satisfying
  \ref{T}--\ref{A}, \ref{p3d}\  holds if, and only if, \ref{c2d}\
  does.
\end{observation}
...
% def-testworthy
\begin{definition*}\label{def-testworthy} A proper $Y$-generalization $\ext$ is
  \emph{testworthy} if $Y$ is of cardinality $2, 3$ or $4$ and it satisfies
  \textup{\ref{K}--\ref{A}} and $\extb _{\novel} = \extb_{D}^{-1}$ for some
  $D\in \mbbd$ such that $\ext_{D}$ is total.\footnote{Recall that the inverse
    $\ext _{D}^{- 1}$ of $\ext _{D}$ satisfies $x \ext _{D}^{- 1} y$ if, and
  only if, $y \ext _{D} x$.}
\end{definition*}
...
\begin{definition*} Let $\ext$ and $\aext$ be generalizations of
  ...
  $\aextb_\mbbd = \extb_\mbbd$ and $\aextb_{\novel} = \extb_{\novel}$. 
  Moreover, the perturbation $\aext$ is \emph{{{diverse}}}  $\countof \total
  (\aextb) \geq \countof \total ( \extb)$.
...
\begin{taggedstability}{\textsc{$\novel$-stability}}\label{S} Every testworthy
  $Y$-generalization $\ext$ has a {{diverse}} perturbation $\aext$ satisfying
  \ref{T}–\ref{A} and $\aextb_\mbbd = \preceqb_{\mbbd}\cap Y^2$.
\end{taggedstability}
...
\begin{linenomath*} 
  \begin{equation}\tag{$*$}\label{eq-rep-main} \left\{
...
equal. Let $\mathbf{v}_Y : Y \times \mbbc \rightarrow \R$, $(x, c) \mapsto
\mathbf{v}(x, c)$ denote the restriction of $\mathbf{v}$ to  $Y\times \mbbc$.
Then, for any $Y$-generalization $\ext$, $\mathbf{v}_Y$ is a representation of
$\ext_\mbbd$ if, \emph{mutatis mutandis}, it satisfies
\eqref{eq-rep-main}.
% whenever,
...
\begin{theorem}[Part I, Existence]\label{thm-main} Let there be given $X$,
  ...
  \begin{enumerate}[label=\textup{(\ref{thm-main}.\roman*)}]
    ...
    \item\label{wrap-main} There exists a matrix $\mathbf{v} : X \times \mbbc
      \rightarrow \R$ satisfying \ref{rep-main}--\ref{genrep-main}$\,:$
      \begin{enumerate}[label=\textup{(\ref{thm-main}.\alph*)}]
        ...
        \item\label{genrep-main} for every testworthy $Y$-generalization $\ext$,
        $\mathbf{v}_Y$ is a representation of $\ext_\mbbd$.
      \end{enumerate}
  \end{enumerate}
\end{theorem}
% uniqueness
...
\begin{enumerate}
  \item\label{eg-supervised-resample} Derive a collection $\mc D_n$ of training
    sets by resampling from the current data $D^{\star}$. (To start with, take
    $n$ ``basis'' training sets each of which contains just a single copy of
    some case in $D^{\star}$.)
  \item\label{eg-supervised-sentiments}Induce sentiments on $\mc D_n$. That is
    a plausibility ranking on $Y$ for each $D$ in $\mc D_n$. This 
    % , so that, given
  ...
  \item \label{eg-supervised-axioms} Via \ref{p3d} and \stability, extend
    sentiments to the set $\mbbd$ of every finite resampling of $D^{\star}$.
  ...
  \item \label{eg-supervised-estimate} Via the groupoid identity, derive a
    ...
    \cref{thm-mainQ}.
  \item Estimate $\rho_x$ using $\hat\rho: Y \times \mbbd \rightarrow \R$, $y
    \times D \mapsto \hat \rho_D(y) = \sum_{c \,\in D} \mathbf{v}(y, c)$, 
    %for the kernel $k = \lvert D\rvert \cdot v$ where 
    where $v$ satisfies the properties of \cref{thm-main}.
...
Before turning to the interpretation of $\preceq_\mbbd$, 
...
\bibliography{ensemble-learning-jabref.bib} % Bibliography file (usually '*.bib')
...
\begin{appendix}
  ...
  \begin{proofEnd}%[Proof of \cref{lem-axiomsQ}]
    ...
    \cref{lem-coneQ}, $\extb_{L} = \extb_{J}$ and $\extb_{L'} = \extb_{J'}$. 
    Moreover, by construction, for some $D$ and $D'$ such that $L_{D}=L$ and
    $I_{D'}= L'$, $\hextb_{D} = \extb_J$ and $\hextb_{D'} = \extb_{J'}$.  We
    therefore conclude that $x \hsext_{D} y$, so that \ref{A} implies the
    existence of $\kappa \in \posint$ and $\{D_{l}: D_{l} \sim^{\hextb}
      D\}_{1}^{\kappa}$ such that $x \hsext_{D_{1} \cup \cdots \cup D_{\kappa}
